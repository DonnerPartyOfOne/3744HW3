...
public class HW3Controller {
	...
	public HW3Controller(HW3Model m, HW3View v) {
		...
		selection.addListener(new ChangeListener<HW2Circle>() { // 
			 ...
			 */
			@Override
			public void changed(ObservableValue<? extends HW2Circle> observable, HW2Circle oldValue, HW2Circle newValue) { repaint(); }
		});
		model.drawDataProperty().addListener(new ListChangeListener<HW3Model.HW2Circle>() {
			 ...
			 */
			@Override
			public void onChanged(ListChangeListener.Change<? extends HW3Model.HW2Circle> e) { 
				//TODO when the model changes, modify the tableview
			...
			public void changed(ObservableValue<? extends HW3View.ViewState> observable, HW3View.ViewState oldValue, HW3View.ViewState newValue) {
				if (!newValue.equals(oldValue)) {
					...
					case SAVE: // Save the file.
						if (view.getFile() != null) {
							...
							try {
								...
								for (HW3Model.HW2Circle c : model.drawDataProperty()) {
					...
					case QUIT: // Quit the application
						if (oldValue == ViewState.MODIFIED && !view.showChoiceDialog(CHOICE_TITLE, CHOICE_MESSAGE)) {
	 ...
	 */
	private HW3Model.HW2Circle getSelection() { return selection.get(); }
	 ...
	 */
	private void setSelection(HW3Model.HW2Circle s) {
		selection.set(s);
	...
	private void reTable() {
		TableView<HW2Circle> tv = view.getTV();
		ObservableList<HW2Circle> circles = model.drawDataProperty();
		tv.setItems(circles);		
	}
